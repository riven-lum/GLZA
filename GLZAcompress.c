...
void move_to_match_sibling(struct match_node *match_nodes, struct match_node **match_node_ptr_ptr,
    uint32_t symbol, uint8_t * sibling_number) {
  uint32_t shifted_symbol = symbol;
  *sibling_number = (uint8_t)(shifted_symbol & 0xF);
  while ((symbol != (*match_node_ptr_ptr)->symbol) && ((*match_node_ptr_ptr)->sibling_node_num[*sibling_number] != 0)) {
...
void move_to_search_sibling(struct match_node *match_nodes, uint32_t symbol, struct match_node **search_node_ptr_ptr) {
  uint8_t sibling_depth = 0;
  uint32_t shifted_symbol = symbol;
  uint8_t sibling_nibble = (uint8_t)(shifted_symbol & 0xF);
  while ((symbol != (*search_node_ptr_ptr)->symbol) && ((*search_node_ptr_ptr)->sibling_node_num[sibling_nibble] != 0)) {
...
void move_to_match_child_with_make(struct match_node *match_nodes, struct match_node **match_node_ptr_ptr,
  ...
  if ((*match_node_ptr_ptr)->child_ptr == 0) {
    ...
    move_to_match_sibling(match_nodes, match_node_ptr_ptr, symbol, &sibling_number);
    if (symbol != (*match_node_ptr_ptr)->symbol) {
...
void write_siblings_miss_ptr(struct match_node *match_nodes, struct match_node *child_ptr,
    struct match_node **search_node_ptr_ptr) {
  uint8_t sibling_nibble;
  child_ptr->miss_ptr = (*search_node_ptr_ptr)->child_ptr;
  for (sibling_nibble = 0 ; sibling_nibble < 16 ; sibling_nibble++) {
    uint32_t sibling_node_number = child_ptr->sibling_node_num[sibling_nibble];
    if (sibling_node_number != 0)
      write_siblings_miss_ptr(match_nodes, &match_nodes[sibling_node_number], search_node_ptr_ptr);
  }
  return;
}


void write_all_children_miss_ptr(struct match_node *match_nodes, struct match_node *match_node_ptr,
    struct match_node **search_node_ptr_ptr) {
  uint8_t sibling_nibble;
  struct match_node * child_match_node_ptr = match_node_ptr->child_ptr;
  if (child_match_node_ptr->miss_ptr == 0) {
    child_match_node_ptr->miss_ptr = (*search_node_ptr_ptr)->child_ptr;
    for (sibling_nibble = 0 ; sibling_nibble < 16 ; sibling_nibble++) {
      uint32_t sibling_node_number = child_match_node_ptr->sibling_node_num[sibling_nibble];
      if (sibling_node_number != 0)
        write_siblings_miss_ptr(match_nodes, &match_nodes[sibling_node_number], search_node_ptr_ptr);
    }
  }
...
struct string_node * create_suffix_node(struct string_node * string_nodes, uint32_t suffix_symbol, uint32_t symbol_index,
...
struct string_node * split_node_for_overlap(struct string_node * string_nodes, struct string_node * node_ptr,
    uint32_t split_index, uint32_t * start_symbol_ptr, uint32_t * in_symbol_ptr, uint32_t * next_string_node_num_ptr) {
...
void add_word_suffix(struct string_node * string_nodes, uint32_t *start_symbol_ptr,
    uint32_t *in_symbol_ptr, int32_t *base_string_nodes_child_node_num, uint32_t *next_string_node_num_ptr) {
  ...
  if (*base_node_child_num_ptr == 0) { // first occurence of the symbol, so create a child
    ...
    *base_node_child_num_ptr = *next_string_node_num_ptr;
    (void)create_suffix_node(string_nodes, *(start_symbol_ptr + symbol_index), symbol_index, next_string_node_num_ptr);
  }
    ...
    do {
      ...
      if (*sibling_node_num_ptr == 0) { // no match so add sibling
        *sibling_node_num_ptr = *next_string_node_num_ptr;
        (void)create_suffix_node(string_nodes, search_symbol, in_symbol_ptr - start_symbol_ptr, next_string_node_num_ptr);
        return;
      }
    ...
    if (num_extra_symbols != 0) {
      ...
      do {
        if (*(node_symbol_ptr + length) != *(in_symbol_ptr + length)) { // insert node in branch
          ...
          (void)create_suffix_node(string_nodes, *(in_symbol_ptr + length),
              in_symbol_ptr + length - start_symbol_ptr, next_string_node_num_ptr);
          node_ptr->instances++;
          return;
        }
      ...
      do {
        ...
        if (*prior_node_num_ptr == 0) {
          *prior_node_num_ptr = *next_string_node_num_ptr;
          (void)create_suffix_node(string_nodes, search_symbol, in_symbol_ptr - start_symbol_ptr,
  ...
  if ((*(node_symbol_ptr + 1) == *(in_symbol_ptr + 1)) && (*in_symbol_ptr != 0x20)
    ...
    while ((*(node_symbol_ptr + length) == *(in_symbol_ptr + length)) && (*(in_symbol_ptr + length - 1) != 0x20)
      ...
      length++;
    node_ptr->num_extra_symbols = length - 1;
    node_ptr = create_suffix_node(string_nodes, *(node_symbol_ptr + length),
        node_symbol_ptr + length - start_symbol_ptr, next_string_node_num_ptr);
    node_ptr->sibling_node_num[*(in_symbol_ptr + length) & 1] = *next_string_node_num_ptr;
    (void)create_suffix_node(string_nodes, *(in_symbol_ptr + length), in_symbol_ptr + length - start_symbol_ptr,
        next_string_node_num_ptr);
  }
  else {
    node_ptr = create_suffix_node(string_nodes, *(node_symbol_ptr + 1), node_symbol_ptr + 1 - start_symbol_ptr,
        next_string_node_num_ptr);
    node_ptr->sibling_node_num[*(in_symbol_ptr + 1) & 1] = *next_string_node_num_ptr;
    (void)create_suffix_node(string_nodes, *(in_symbol_ptr + 1), in_symbol_ptr + 1 - start_symbol_ptr,
        next_string_node_num_ptr);
  }
  return;
}
...
void add_suffix(struct string_node * string_nodes, uint32_t first_symbol, uint32_t *start_symbol_ptr,
    uint32_t *in_symbol_ptr, int32_t *base_string_nodes_child_node_num, uint32_t *next_string_node_num_ptr) {
  struct string_node * node_ptr;
  uint32_t search_symbol = *in_symbol_ptr;
  ...
  if (*base_node_child_num_ptr < 0) {
    ...
    *base_node_child_num_ptr = *next_string_node_num_ptr;
    node_ptr = create_suffix_node(string_nodes, old_symbol, symbol_index, next_string_node_num_ptr);
    ...
    if ((*(node_symbol_ptr + 1) == *(in_symbol_ptr + 1)) && (in_symbol_ptr < max_symbol_ptr)) {
      ...
      if (node_ptr->last_match_index + length <= string_start_index) {
        ...
        node_ptr = split_node_for_overlap(string_nodes, node_ptr, string_start_index, start_symbol_ptr, in_symbol_ptr,
            next_string_node_num_ptr);
      node_ptr->child_node_num = *next_string_node_num_ptr;
      node_ptr = create_suffix_node(string_nodes, *(node_symbol_ptr + length),
          node_symbol_ptr + length - start_symbol_ptr, next_string_node_num_ptr);
  ...
  if (search_symbol != node_ptr->symbol) {  // follow siblings until match found or end of siblings found
    ...
    do {
      ...
      if (*sibling_node_num_ptr < 0) { // turn the sibling into a node
        ...
        *sibling_node_num_ptr = *next_string_node_num_ptr;
        node_ptr = create_suffix_node(string_nodes, *(start_symbol_ptr + symbol_index), symbol_index,
            next_string_node_num_ptr);
      ...
      do {
        if (*(node_symbol_ptr + length) != *(in_symbol_ptr + length)) { // insert node in branch
          ...
          if (new_node_lmi <= string_start_index) {
            ...
            (void)split_node_for_overlap(string_nodes, node_ptr, string_start_index, start_symbol_ptr, in_symbol_ptr,
                next_string_node_num_ptr);
          return;
        }
      } while (length++ != num_extra_symbols);
...
void add_suffix_count(struct string_node * string_nodes, uint32_t first_symbol, uint32_t *start_symbol_ptr,
    uint32_t *in_symbol_ptr, int32_t *base_string_nodes_child_node_num, uint32_t *next_string_node_num_ptr) {
  struct string_node * node_ptr;
  uint32_t search_symbol = *in_symbol_ptr;
  if ((int32_t)search_symbol < 0)
...
void *build_lcp_thread(void *arg) {
  ...
  struct string_node * string_nodes = thread_data_ptr->string_nodes;
  uint32_t * start_symbol_ptr = thread_data_ptr->start_symbol_ptr;
    ...
    if (in_symbol_ptr == local_scan_symbol_ptr)
      ...
      do {
        ...
        if ((symbol >= min_symbol) && (symbol <= max_symbol)) {
          add_suffix(string_nodes, symbol, start_symbol_ptr, in_symbol_ptr, base_string_nodes_child_node_num,
              &next_string_node_num);
...
void *rank_scores_thread(void *arg) {
  ...
  while (1) {
    ...
    if (score > min_score) {
      ...
      uint32_t new_score_smi1_m_1, new_score_smi2_m_1;
      if (new_score_lmi2 < new_score_lmi1) {
        uint32_t temp_lmi = new_score_lmi1;
        new_score_lmi1 = new_score_lmi2;
        new_score_lmi2 = temp_lmi;
      }
      new_score_smi2_m_1 = new_score_lmi2 - num_symbols;
      new_score_smi1_m_1 = new_score_lmi1 - num_symbols;
      score_position = 0;
...
void score_base_node_tree(struct string_node *string_nodes, struct string_node *node_ptr, struct score_data *node_data,
    uint32_t *start_symbol_ptr, double string_entropy, double production_cost, double profit_ratio_power,
    double log2_num_symbols_plus_substitution_cost, double *new_symbol_cost, double *symbol_entropy,
    struct node_score_data *rank_scores_buffer, uint16_t *node_ptrs_num_ptr, uint8_t UTF8_compliant) {
...
void score_base_node_tree_cap(struct string_node *string_nodes, struct string_node *node_ptr,
    struct score_data *node_data, uint32_t *start_symbol_ptr, double string_entropy, double production_cost,
    double profit_ratio_power, double log2_num_symbols_plus_substitution_cost, double *new_symbol_cost,
...
void score_base_node_tree_cap_prp3(struct string_node *string_nodes, struct string_node* node_ptr,
    struct score_data *node_data, uint32_t *start_symbol_ptr, double string_entropy, double production_cost,
    double log2_num_symbols_plus_substitution_cost, double *new_symbol_cost, double *symbol_entropy,
...
void score_base_node_tree_prp2(struct string_node *string_nodes, struct string_node* node_ptr,
    struct score_data *node_data, uint32_t *start_symbol_ptr, double string_entropy, double production_cost,
    double log2_num_symbols_plus_substitution_cost, double *new_symbol_cost, double *symbol_entropy,
    struct node_score_data *rank_scores_buffer, uint16_t *node_ptrs_num_ptr, uint8_t UTF8_compliant) {
  uint16_t num_symbols = 2;
...
void score_base_node_tree_cap_prp2(struct string_node *string_nodes, struct string_node* node_ptr,
    struct score_data *node_data, uint32_t *start_symbol_ptr, double string_entropy, double production_cost,
    double log2_num_symbols_plus_substitution_cost, double *new_symbol_cost, double *symbol_entropy,
    struct node_score_data *rank_scores_buffer, uint16_t *node_ptrs_num_ptr) {
  uint16_t num_symbols = 2;
...
void score_base_node_tree_prp1(struct string_node *string_nodes, struct string_node* node_ptr,
    struct score_data *node_data, uint32_t *start_symbol_ptr, double string_entropy, double production_cost,
    double log2_num_symbols_plus_substitution_cost, double *new_symbol_cost, double *symbol_entropy,
    struct node_score_data *rank_scores_buffer, uint16_t *node_ptrs_num_ptr, uint8_t UTF8_compliant) {
  uint16_t num_symbols = 2;
...
void score_base_node_tree_cap_prp1(struct string_node *string_nodes, struct string_node* node_ptr,
    struct score_data *node_data, uint32_t *start_symbol_ptr, double string_entropy, double production_cost,
    double log2_num_symbols_plus_substitution_cost, double *new_symbol_cost, double *symbol_entropy,
    struct node_score_data *rank_scores_buffer, uint16_t *node_ptrs_num_ptr) {
  uint16_t num_symbols = 2;
...
void score_base_node_tree_prp0(struct string_node *string_nodes, struct string_node* node_ptr,
    struct score_data *node_data, uint32_t *start_symbol_ptr, double string_entropy, double production_cost,
    double log2_num_symbols_plus_substitution_cost, double *new_symbol_cost, double *symbol_entropy,
    struct node_score_data *rank_scores_buffer, uint16_t *node_ptrs_num_ptr, uint8_t UTF8_compliant) {
  uint16_t num_symbols = 2;
...
void score_base_node_tree_cap_prp0(struct string_node *string_nodes, struct string_node* node_ptr,
    struct score_data *node_data, uint32_t *start_symbol_ptr, double string_entropy, double production_cost,
    double log2_num_symbols_plus_substitution_cost, double *new_symbol_cost, double *symbol_entropy,
    struct node_score_data *rank_scores_buffer, uint16_t *node_ptrs_num_ptr) {
  uint16_t num_symbols = 2;
...
void score_base_node_tree_words(struct string_node *string_nodes, struct string_node* node_ptr,
    struct score_data *node_data, uint32_t *start_symbol_ptr, double production_cost,
    double log2_num_symbols_plus_substitution_cost, double *new_symbol_cost, double *symbol_entropy,
    struct node_score_data *rank_scores_buffer, uint16_t *node_ptrs_num_ptr) {
...
void score_symbol_tree(struct string_node *string_nodes, uint32_t min_symbol, uint32_t max_symbol,
    uint32_t *start_symbol_ptr, struct node_score_data *rank_scores_buffer, struct score_data *node_data,
    int32_t *base_string_nodes_child_node_num, uint16_t *node_ptrs_num_ptr, double production_cost,
    double profit_ratio_power, double log2_num_symbols_plus_substitution_cost, double *new_symbol_cost,
    double *symbol_entropy, uint8_t cap_encoded, uint8_t UTF8_compliant, uint32_t * symbol_counts) {
    ...
    if (profit_ratio_power == 3.0) {
      while (symbol <= max_symbol) {
        if (symbol_counts[symbol] > 1) {
          ...
          do {
            if (*base_node_child_num_ptr > 0)
              score_base_node_tree_cap_prp3(string_nodes, &string_nodes[*base_node_child_num_ptr], node_data,
                  start_symbol_ptr, symbol_entropy[symbol], production_cost, log2_num_symbols_plus_substitution_cost,
                  new_symbol_cost, symbol_entropy, rank_scores_buffer, node_ptrs_num_ptr);
      ...
      while (symbol <= max_symbol) {
        if (symbol_counts[symbol] > 1) {
          ...
          do {
            if (*base_node_child_num_ptr > 0)
              score_base_node_tree_cap_prp2(string_nodes, &string_nodes[*base_node_child_num_ptr], node_data,
                  start_symbol_ptr, symbol_entropy[symbol], production_cost, log2_num_symbols_plus_substitution_cost,
                  new_symbol_cost, symbol_entropy, rank_scores_buffer, node_ptrs_num_ptr);
            base_node_child_num_ptr++;
          } while (base_node_child_num_ptr != next_base_node_child_num_ptr);
      ...
      while (symbol <= max_symbol) {
        if (symbol_counts[symbol] > 1) {
          ...
          do {
            if (*base_node_child_num_ptr > 0)
              score_base_node_tree_cap_prp1(string_nodes, &string_nodes[*base_node_child_num_ptr], node_data,
                  start_symbol_ptr, symbol_entropy[symbol], production_cost, log2_num_symbols_plus_substitution_cost,
                  new_symbol_cost, symbol_entropy, rank_scores_buffer, node_ptrs_num_ptr);
            base_node_child_num_ptr++;
          } while (base_node_child_num_ptr != next_base_node_child_num_ptr);
      ...
      while (symbol <= max_symbol) {
        if (symbol_counts[symbol] > 1) {
          ...
          do {
            if (*base_node_child_num_ptr > 0)
              score_base_node_tree_cap_prp0(string_nodes, &string_nodes[*base_node_child_num_ptr], node_data,
                  start_symbol_ptr, symbol_entropy[symbol], production_cost, log2_num_symbols_plus_substitution_cost,
                  new_symbol_cost, symbol_entropy, rank_scores_buffer, node_ptrs_num_ptr);
            base_node_child_num_ptr++;
          } while (base_node_child_num_ptr != next_base_node_child_num_ptr);
      ...
      while (symbol <= max_symbol) {
        if (symbol_counts[symbol] > 1) {
          ...
          do {
            if (*base_node_child_num_ptr > 0)
              score_base_node_tree_cap(string_nodes, &string_nodes[*base_node_child_num_ptr], node_data, start_symbol_ptr,
                  symbol_entropy[symbol], production_cost, profit_ratio_power, log2_num_symbols_plus_substitution_cost,
                  new_symbol_cost, symbol_entropy, rank_scores_buffer, node_ptrs_num_ptr);
            base_node_child_num_ptr++;
          } while (base_node_child_num_ptr != next_base_node_child_num_ptr);
        }
    ...
    if (profit_ratio_power == 2.0) {
      while (symbol <= max_symbol) {
        if (symbol_counts[symbol] > 1) {
          ...
          do {
            if (*base_node_child_num_ptr > 0)
              score_base_node_tree_prp2(string_nodes, &string_nodes[*base_node_child_num_ptr], node_data, start_symbol_ptr,
      ...
      while (symbol <= max_symbol) {
        if (symbol_counts[symbol] > 1) {
          ...
          do {
            if (*base_node_child_num_ptr > 0)
              score_base_node_tree_prp1(string_nodes, &string_nodes[*base_node_child_num_ptr], node_data, start_symbol_ptr,
                  symbol_entropy[symbol], production_cost, log2_num_symbols_plus_substitution_cost, new_symbol_cost,
                  symbol_entropy, rank_scores_buffer, node_ptrs_num_ptr, UTF8_compliant);
            base_node_child_num_ptr++;
          } while (base_node_child_num_ptr != next_base_node_child_num_ptr);
      ...
      while (symbol <= max_symbol) {
        if (symbol_counts[symbol] > 1) {
          ...
          do {
            if (*base_node_child_num_ptr > 0)
              score_base_node_tree_prp0(string_nodes, &string_nodes[*base_node_child_num_ptr], node_data, start_symbol_ptr,
                  symbol_entropy[symbol], production_cost, log2_num_symbols_plus_substitution_cost, new_symbol_cost,
                  symbol_entropy, rank_scores_buffer, node_ptrs_num_ptr, UTF8_compliant);
            base_node_child_num_ptr++;
          } while (base_node_child_num_ptr != next_base_node_child_num_ptr);
      ...
      while (symbol <= max_symbol) {
        if (symbol_counts[symbol] > 1) {
          ...
          do {
            if (*base_node_child_num_ptr > 0)
              score_base_node_tree(string_nodes, &string_nodes[*base_node_child_num_ptr], node_data, start_symbol_ptr,
                  symbol_entropy[symbol], production_cost, profit_ratio_power, log2_num_symbols_plus_substitution_cost,
                  new_symbol_cost, symbol_entropy, rank_scores_buffer, node_ptrs_num_ptr, UTF8_compliant);
            base_node_child_num_ptr++;
          } while (base_node_child_num_ptr != next_base_node_child_num_ptr);
        }
        else
...
void score_symbol_tree_words(struct string_node *string_nodes, uint32_t *start_symbol_ptr,
  ...
  int32_t * base_node_child_num_end_ptr = &base_string_nodes_child_node_num[0x80 + BASE_NODES_CHILD_ARRAY_SIZE];
  do {
    int32_t base_node = *base_node_child_num_ptr++;
    if (base_node > 0)
      score_base_node_tree_words(string_nodes, &string_nodes[base_node], node_data, start_symbol_ptr, production_cost,
...
thread_overlap_check_loop_no_match:
  ...
  if (match_nodes[symbol].num_symbols == 0)
    goto thread_overlap_check_loop_no_match;
  match_node_ptr = &match_nodes[symbol];
...
void *find_substitutions_thread(void *arg) {
  ...
  uint32_t symbol, node_score_number;
  struct match_node *match_node_ptr;
...
thread_symbol_substitution_loop_match:
      symbol = *in_symbol_ptr++;
      if ((int32_t)symbol >= 0) {
        match_node_ptr = match_node_ptr->child_ptr;
        if (symbol != match_node_ptr->symbol) {
            ...
            if (match_node_ptr->sibling_node_num[sibling_nibble & 0xF] != 0) {
              ...
              if (match_node_ptr->miss_ptr == 0) {
                if (match_nodes[symbol].num_symbols != 0) {
                  ...
                  match_node_ptr = &match_nodes[symbol];
                  goto thread_symbol_substitution_loop_match;
                }
                if (in_symbol_ptr >= end_symbol_ptr) {
                  num_symbols_to_copy += match_node_ptr->num_symbols - (in_symbol_ptr - end_symbol_ptr);
                  goto thread_symbol_substitution_loop_end;
...
void *substitute_thread(void *arg) {
  ...
  uint32_t * start_symbol_ptr = thread_data_ptr->symbol_ptr;
  ...
  while (1) {
    if ((local_write_index = atomic_load_explicit(&substitute_data_write_index, memory_order_relaxed))
      ...
      do {
        if ((int32_t)(data = substitute_data[substitute_data_index++]) > 0) {
          ...
          thread_data_ptr->symbol_ptr = out_symbol_ptr;
          return(0);
        }
        else {
...
uint8_t GLZAcompress(size_t in_size, size_t * outsize_ptr, uint8_t ** iobuf, struct param_data * params) {
  ...
  uint32_t symbol, num_symbols_to_copy, next_new_symbol_number, initial_max_scores, max_scores;
  ...
  uint32_t *start_symbol_ptr, *in_symbol_ptr, *out_symbol_ptr, *stop_symbol_ptr, *end_symbol_ptr;
  ...
  int32_t *base_string_nodes_child_node_num, *base_node_child_num_ptr;
  ...
  float prior_min_score, new_min_score, cycle_start_ratio, cycle_end_ratio;
  ...
  pthread_t build_lcp_threads[6], rank_scores_thread1, substitute_thread1;
  ...
  *end_symbol_ptr = 0xFFFFFFFE;
  ...
  if ((0 == (rank_scores_data_ptr = (struct rank_scores_thread_data *)malloc(sizeof(struct rank_scores_thread_data))))
      || (0 == (new_symbol_number = (uint32_t *)malloc(MAX_SCORES * sizeof(uint32_t))))
  ...
  struct node_score_data * candidates = &rank_scores_data_ptr->candidates[0];
  ...
  scan_cycle = 0;
  scan_mode = 0;
  if ((cap_encoded == 0) || (create_words == 0))
    scan_mode = 1;
...
top_main_loop:
    ...
    struct string_node * string_nodes = (struct string_node *)((size_t)free_RAM_ptr
    ...
    if (scan_mode == 0) {
      ...
      do {
        ...
        if (symbol == 0x20) {
          if (((*in_symbol_ptr >= (uint32_t)'a') && (*in_symbol_ptr <= (uint32_t)'z'))
            ...
            add_word_suffix(string_nodes, start_symbol_ptr, in_symbol_ptr, base_string_nodes_child_node_num,
                &next_string_node_num);
        }
      ...
      score_symbol_tree_words(string_nodes, start_symbol_ptr, rank_scores_data_ptr->rank_scores_buffer,
          node_data, base_string_nodes_child_node_num, &node_ptrs_num, production_cost,
          log2_num_symbols_plus_substitution_cost, new_symbol_cost, symbol_entropy);
      ...
      float min_score = (float)(1000.0 + 400.0 * (log2(order_0_entropy + 3000000.0) - log2(3000000.0)));
      ...
      if ((num_candidates != 0) && (candidates[candidates_index[0]].score >= min_score)) {
        ...
        while (i1 < num_candidates) {
          ...
          if (candidates[candidates_index[i1]].score < min_score)
            num_candidates = i1;
          if (free_RAM_ptr + num_match_nodes * sizeof(struct match_node) + max_match_length * sizeof(uint32_t)
              >= end_RAM_ptr)
            num_candidates = i1 - 1;
          i1++;
        }
        ...
        do {
            ...
            if (candidate_bad[i1] == 0) {
              ...
              while (best_score_match_ptr <= best_score_last_match_ptr) {
                ...
                if (match_node_ptr->child_ptr == 0) {
                  ...
                  match_node_ptr = match_node_ptr->child_ptr;
                  init_match_node(match_node_ptr, symbol, best_score_num_symbols, i1);
                }
                else {
                  match_node_ptr = match_node_ptr->child_ptr;
                  uint8_t sibling_number;
                  move_to_match_sibling(match_nodes, &match_node_ptr, symbol, &sibling_number);
                  if (symbol == match_node_ptr->symbol) {
                    if (match_node_ptr->child_ptr == 0)
          ...
          // Redo the tree build with the final valid score symbols
          num_match_nodes = next_new_symbol_number;
          ...
          while (i1 < num_candidates) {
            if (candidate_bad[i1] == 0) {
              ...
              match_node_ptr = &match_nodes[0x20];
              if (match_node_ptr->num_symbols == 0)
                init_match_node(match_node_ptr, 0x20, 1, i1);
              while (best_score_match_ptr <= best_score_last_match_ptr) {
                symbol = *best_score_match_ptr++;
              ...
              symbol_counts[num_simple_symbols + i2] = 0;
          ...
          if (num_file_symbols >= 1000000) {
            stop_symbol_ptr = start_symbol_ptr + (num_file_symbols >> 3);
            find_substitutions_thread_data[0].start_symbol_ptr = stop_symbol_ptr;
            block_size = num_file_symbols / 7;
            ...
            for (i1 = 0 ; i1 < 6 ; i1++) {
              ...
              atomic_store_explicit(&find_substitutions_thread_data[i1].read_index, 0, memory_order_release);
              atomic_store_explicit(&find_substitutions_thread_data[i1].write_index, 0, memory_order_release);
          ...
          substitute_thread_data.symbol_ptr = start_symbol_ptr;
...
wmain_symbol_substitution_loop_match:
            symbol = *in_symbol_ptr++;
            if ((int32_t)symbol >= 0) {
              match_node_ptr = match_node_ptr->child_ptr;
              if (symbol != match_node_ptr->symbol) {
                ...
                do {
                  if (match_node_ptr->sibling_node_num[sibling_nibble & 0xF] != 0) {
                    ...
                    if (symbol == 0x20) {
                      ...
                      match_node_ptr = &match_nodes[0x20];
                      goto wmain_symbol_substitution_loop_match;
                    }
                    if (in_symbol_ptr >= stop_symbol_ptr) {
                      num_symbols_to_copy += match_node_ptr->num_symbols - (in_symbol_ptr - stop_symbol_ptr);
                      goto wmain_symbol_substitution_loop_end;
              ...
              if (((substitute_index + 1) & 0x7FFE) == 0)
                while ((uint16_t)(substitute_index - atomic_load_explicit(&substitute_data_read_index,
                    memory_order_acquire)) >= 0x7FFF) /* wait */ ;
...
wmain_symbol_substitution_loop_end:
              ...
              while ((atomic_load_explicit(&find_substitutions_thread_data[i1].done, memory_order_acquire) == 0)
                ...
                if (substitutions_index != local_substitutions_write_index) {
                  while (substitutions_index != local_substitutions_write_index) {
                    if ((substitute_index & 0x3FFF) == 0) {
                      atomic_store_explicit(&find_substitutions_thread_data[i1].read_index, substitutions_index,
                          memory_order_release);
                      while ((uint16_t)(substitute_index - atomic_load_explicit(&substitute_data_read_index,
                          memory_order_acquire)) >= 0xC000) /* wait */ ;
                    }
                    if ((int32_t)find_substitutions_thread_data[i1].data[substitutions_index] < 0) {
                      substitute_data[substitute_index++] = find_substitutions_thread_data[i1].data[substitutions_index];
                      substitutions_index = (substitutions_index + 1) & 0x7FFFF;
                    }
                    else {
                      substitute_data[substitute_index++] = find_substitutions_thread_data[i1].data[substitutions_index];
                      substitutions_index = (substitutions_index + 1) & 0x7FFFF;
          ...
          out_symbol_ptr = substitute_thread_data.symbol_ptr;
    ...
    for (i1 = 0 ; i1 < 12 ; i1++) {
      lcp_thread_data[i1].string_nodes = string_nodes;
      lcp_thread_data[i1].start_symbol_ptr = start_symbol_ptr;
    ...
    if (cycle_size_limit == 0.0) {
      ...
      do {
        ...
        if (symbol <= main_max_symbol) {
          atomic_store_explicit(&scan_symbol_ptr, (uintptr_t)in_symbol_ptr, memory_order_relaxed);
          add_suffix_count(string_nodes, symbol, start_symbol_ptr, in_symbol_ptr, base_string_nodes_child_node_num,
              &next_string_node_num);
          num_lengths++;
...
done_building_lcp_tree:
    ...
    fprintf(stderr,"                                              \r.");
#endif
    score_symbol_tree(string_nodes, 0, main_max_symbol, start_symbol_ptr, rank_scores_data_ptr->rank_scores_buffer,
        node_data, base_string_nodes_child_node_num, &node_ptrs_num, production_cost, profit_ratio_power,
        log2_num_symbols_plus_substitution_cost, new_symbol_cost, symbol_entropy, cap_encoded, UTF8_compliant, symbol_counts);
#ifdef PRINTON
    fprintf(stderr,".");
#endif
    ...
    fprintf(stderr,".");
#endif
    score_symbol_tree(string_nodes, main_max_symbol + 1, lcp_thread_data[0].max_symbol, start_symbol_ptr,
        rank_scores_data_ptr->rank_scores_buffer, node_data, base_string_nodes_child_node_num, &node_ptrs_num,
    ...
    for (i1 = 1 ; i1 <= 5 ; i1++) {
      ...
      fprintf(stderr,".");
#endif
      score_symbol_tree(string_nodes, lcp_thread_data[i1-1].max_symbol + 1, lcp_thread_data[i1].max_symbol,
          start_symbol_ptr, rank_scores_data_ptr->rank_scores_buffer, node_data, base_string_nodes_child_node_num,
          &node_ptrs_num, production_cost, profit_ratio_power, log2_num_symbols_plus_substitution_cost, new_symbol_cost,
          symbol_entropy, cap_encoded, UTF8_compliant, symbol_counts);
    }
    ...
    if (num_candidates != 0) {
      ...
      if ((cycle_size_limit == 0.0) &&
          (candidates[candidates_index[num_candidates - 1]].score < 0.1 * candidates[candidates_index[0]].score - 1.0)) {
      ...
      i1 = 0;
      while (i1 < num_candidates) {
        if (free_RAM_ptr + (next_new_symbol_number + num_match_nodes - 1) * sizeof(struct match_node) + max_match_length * sizeof(uint32_t)
            >= end_RAM_ptr)
          num_candidates = i1 - 1;
        if (candidates[candidates_index[i1]].num_symbols > max_match_length)
          max_match_length = candidates[candidates_index[i1]].num_symbols;
        uint32_t *best_score_last_match_ptr, *best_score_match_ptr;
        best_score_last_match_ptr = start_symbol_ptr + candidates[candidates_index[i1]].last_match_index1;
        best_score_match_ptr = best_score_last_match_ptr - candidates[candidates_index[i1]].num_symbols + 1;
        match_node_ptr = match_nodes;
        while (best_score_match_ptr <= best_score_last_match_ptr) {
          symbol = *best_score_match_ptr;
          if (match_node_ptr->child_ptr == 0) {
      ...
      while (i1 < num_candidates) {
        ...
        while (best_score_match_ptr <= best_score_last_match_ptr) {
          ...
          while (1) { // follow the tree until find child = 0 or sibling = 0
            ...
            move_to_search_sibling(match_nodes, symbol, &search_node_ptr);
            if (symbol != search_node_ptr->symbol) // no child match so exit suffix search
              break;
      ...
      while (i1 < num_candidates) {
        if (candidate_bad[i1] == 0) {
          ...
          best_score_last_match_ptr = start_symbol_ptr + candidates[candidates_index[i1]].last_match_index1;
          best_score_suffix_ptr = best_score_last_match_ptr - candidates[candidates_index[i1]].num_symbols + 1;
          ...
          while (best_score_suffix_ptr <= best_score_last_match_ptr) {
            ...
            if (match_nodes[symbol].num_symbols != 0) {
              struct match_node * search_node_ptr = &match_nodes[symbol];
              if (match_node_ptr->child_ptr == 0) {
                if (match_node_ptr->hit_ptr == 0)
                  match_node_ptr->hit_ptr = search_node_ptr;
              }
              else
                write_all_children_miss_ptr(match_nodes, match_node_ptr, &search_node_ptr);
              ...
              while (best_score_match_ptr <= best_score_last_match_ptr) {
                ...
                search_node_ptr = search_node_ptr->child_ptr;
                move_to_search_sibling(match_nodes, symbol, &search_node_ptr);
                if (symbol != search_node_ptr->symbol) // no matching sibling, so done with this suffix
                  break;
                if (match_node_ptr->child_ptr == 0) {
                  if (match_node_ptr->hit_ptr == 0)
                    match_node_ptr->hit_ptr = search_node_ptr;
...
main_overlap_check_loop_match:
      if (in_symbol_ptr - match_node_ptr->num_symbols >= stop_symbol_ptr)
        goto main_overlap_check_loop_end;
      symbol = *in_symbol_ptr++;
      if ((int32_t)symbol < 0)
        goto main_overlap_check_loop_no_match;
...
main_symbol_substitution_loop_top:
      ...
      if ((int32_t)symbol >= 0) {
        match_node_ptr = &match_nodes[symbol];
        if (match_node_ptr->num_symbols != 0) {
main_symbol_substitution_loop_match:
          symbol = *in_symbol_ptr++;
          if ((int32_t)symbol >= 0) {
            match_node_ptr = match_node_ptr->child_ptr;
            if (symbol != match_node_ptr->symbol) {
                ...
                if (match_node_ptr->sibling_node_num[sibling_nibble & 0xF] != 0) {
                  ...
                  if (match_node_ptr->miss_ptr == 0) {
                    if (match_nodes[symbol].num_symbols != 0) {
                      ...
                      match_node_ptr = &match_nodes[symbol];
                      goto main_symbol_substitution_loop_match;
                    }
                    if (in_symbol_ptr >= stop_symbol_ptr) {
                      num_symbols_to_copy += match_node_ptr->num_symbols - (in_symbol_ptr - stop_symbol_ptr);
                      goto main_symbol_substitution_loop_end;
    ...
    while (in_symbol_ptr != end_symbol_ptr) {
      ...
      if (symbol_value < 0x80)
        ...
        *in_char_ptr++ = (uint8_t)(symbol_value & 0xFF);
        ...
        symbol_value -= 0x80000000 + num_simple_symbols;
